...
class LSTM(nn.Module):
    def __init__(self, args):
        ...
        self.embedding_cate = nn.ModuleDict(
            {col : nn.Embedding(num+1, self.hidden_dim//3) for col, num in args.n_embdings.items()}
        )
...
#        test, question, tag, _, mask, interaction = input
        ...
        embed_cate = [embedding(cate[col_name]) for col_name, embedding in self.embedding_cate.items()]
...
class Saint(nn.Module):
    ...
    def forward(self, input):
        ...
        enc_embed_cate = [embedding(cate[col_name]) for col_name, embedding in self.embedding_cate.items()]
        ...
        embed_enc = torch.cat(
            [enc_embed_cate, enc_embed_cont],2
        )
        ...
        dec_embed_cate = [embedding(cate[col_name]) for col_name, embedding in self.embedding_cate.items()]
        ...
        embed_dec = torch.cat(
            [dec_embed_cate, dec_embed_cont],2
        )
